...
export default function EventDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    const fetchEvent = async () => {
      try {
        const response = await fetch(`http://localhost:5001/events/${paramsId}`);
  ...
  const handleRegister = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5001/events/${paramsId}/register`, {
        method: 'POST',
        headers: {
  ...
  const handleCancelRegistration = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5001/events/${paramsId}/unregister`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
